...
async function sendData() {
  try {
    const res1 = await fetch('http://localhost:3005/api/manual_parse', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text: text1 })
    });
    console.log('Yemeksepeti Result:', await res1.json());
    
    const res2 = await fetch('http://localhost:3005/api/manual_parse', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text: text2 })
    });
    console.log('Trendyol Result:', await res2.json());
