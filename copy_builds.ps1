...
$exePath = "C:\Users\bilal\SARACAPP\SARACAPPV3\app1\dist\saracapp2-6.0.18-setup.exe"
$latestYml = "C:\Users\bilal\SARACAPP\SARACAPPV3\app1\dist\latest.yml"
$blockmap = "C:\Users\bilal\SARACAPP\SARACAPPV3\app1\dist\saracapp2-6.0.18-setup.exe.blockmap"
$apkPath = "C:\Users\bilal\SARACAPP\SARACAPPV3\app2\app\build\outputs\apk\release\app-release.apk"

if (Test-Path $exePath) {
    Copy-Item $exePath -Destination "$distDir\SaracAppv6.0.18-setup.exe" -Force
    Copy-Item $exePath -Destination "$distDir\saracapp2-6.0.18-setup.exe" -Force
...
if (Test-Path $blockmap) {
    Copy-Item $blockmap -Destination "$distDir\SaracAppv6.0.18-setup.exe.blockmap" -Force
    Copy-Item $blockmap -Destination "$distDir\saracapp2-6.0.18-setup.exe.blockmap" -Force
    Write-Host "Copied blockmap"
} else { Write-Host "blockmap not found!" }

if (Test-Path $apkPath) {
    Copy-Item $apkPath -Destination "$distDir\SaracAppv6.0.18.apk" -Force
